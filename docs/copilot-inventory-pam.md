# Copilot Inventory — Evidence Gathering (Pam, 2026-07-14)

## (A) Real Usage — Transcripts and Event Logs

**No persisted copilot transcripts exist on disk.**

Searched locations:
- `training-state/` — no copilot-specific logs; only tool job dirs (reward_probe, ram_capture, record_episode) with `output.log` files containing tool execution traces, not copilot conversational exchanges
- `training-state/tools/` — 41 job directories; none reference copilot
- `server.log` — 1227 HTTP request lines matching "copilot", all are polling/management requests (`GET /api/copilot/events?since=N`, `POST /api/copilot/start`, `POST /api/copilot/send`, `POST /api/copilot/stop`) with no message body content logged
- `/tmp/retro-copilot.err.log` — exists, 0 lines
- Grep for "copilot" across `.jsonl`, `.log`, `.md` — no transcript files found
- No files matching `*transcript*` or `*session*` anywhere in the repo

**Where copilot events live:**

The copilot's conversational events are stored **entirely in-memory** as a ring buffer in `backend/copilot.py`:

- `backend/copilot.py:39` — `_events: list = []  # [{seq, ts, kind, text, raw?}]`
- `backend/copilot.py:45-51` — `_emit()` appends to `_events`, then truncates: `del _events[:-500]`
- Event kinds: `user` (user messages), `assistant` (copilot text replies), `tool` (tool calls), `meta` (session lifecycle)
- Exposed via `GET /api/copilot/events?since=N` (line 117) — poll-based, not SSE/WS
- Buffer is capped at 500 events; session does NOT survive server restart; no persistence layer exists

This is confirmed in `docs/studio-v2-as-built.md:53-55`:
> Copilot events are poll-based (1s) not SSE/WS; ring buffer capped at 500 events; raw arg of _emit unused. Session doesn't survive server restart and there's no --resume support yet.

The copilot ran against `Qwen 3.6 27B` via `claude-local` headless CLI (see `backend/copilot.py:82-96`). Games it likely interacted with based on tool job history: F-Zero (SNES), Little Mermaid (NES), Super Mario Bros (NES), Airstriker (Genesis).

**Representative tool jobs** (not conversational transcripts, but evidence of copilot-driven work):

| Job | Type | Notes |
|-----|------|-------|
| `training-state/tools/reward_probe-*/output.log` | 15 reward probes | Evidence of config validation runs |
| `training-state/tools/ram_capture-*/output.log` | 7 RAM captures | Evidence of RAM variable discovery |
| `training-state/tools/record_episode-*/output.log` | 8 episode recordings | Evidence of behavior observation |

---

## (B) Verbatim Primer

Full exact contents of `backend/copilot_primer.md` (397 lines):
