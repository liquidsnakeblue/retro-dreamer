...
def get_log_dir(fabric: Fabric, root_dir: str, run_name: str, share: bool = True) -> str:
    ...
    if fabric.world_size > 1 and share:
        # Force gloo backend for broadcast_object_list to avoid NCCL OOM issues
        # broadcast_object_list is inefficient with NCCL as it causes GPU->CPU transfers
        world_collective.setup(
            backend="gloo"  # Always use gloo for object broadcasting
        )
    ...
    if fabric.is_global_zero:
        ...
        if fabric.world_size > 1 and share:
            # Force gloo backend for object broadcasting to avoid NCCL OOM issues
            import torch.distributed as dist
            if dist.is_initialized() and dist.get_backend() == 'nccl':
                # Create a gloo process group for object broadcasting
                gloo_group = dist.new_group(backend='gloo')
                dist.broadcast_object_list([log_dir], src=0, group=gloo_group)
            else:
