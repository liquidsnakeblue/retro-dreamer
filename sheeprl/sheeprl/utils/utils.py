...
class Ratio:
    ...
    def __call__(self, step: int, max_repeats: int = 100) -> int:
        if self._ratio == 0:
            return 0
        if self._prev is None:
            ...
            # Cap initial burst to prevent stalling at prefill->training transition
            if repeats > max_repeats:
                self._prev = step - max_repeats / self._ratio
                repeats = max_repeats
            return repeats
