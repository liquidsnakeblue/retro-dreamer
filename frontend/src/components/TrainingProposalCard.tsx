...
export type ProposalAction = {
  status: 'pending' | 'confirming' | 'cancelling' | 'confirmed' | 'cancelled' | 'error'
  error?: string
}
...
export function TrainingProposalCard({
  ...
  const settled = action.status === 'confirmed' || action.status === 'cancelled'
  const disabled = !approvalReady || acting || settled || expired
  const mode = proposal.mode === 'new' ? 'New brain' : proposal.mode === 'resume' ? 'Resume brain' : 'Switch game'
  ...
  return (
    <section className="rounded-lg border border-retro-accent/60 bg-retro-surface/80 overflow-hidden shadow-lg shadow-black/20">
      <div className="px-4 py-3 border-b border-retro-border bg-retro-accent/10 flex items-start justify-between gap-3">
        ...
        <span className="shrink-0 rounded border border-retro-border bg-retro-card px-2 py-1 text-[10px] font-semibold text-retro-text">
          {mode}
        </span>
      </div>
      ...
      <div className="p-4 space-y-4">
        <dl className="grid grid-cols-2 xl:grid-cols-4 gap-x-4 gap-y-3">
          <ProposalField label="Checkpoint" value={head} wide />
          <ProposalField label="Model" value={proposal.model.size.toUpperCase()} />
          <ProposalField label="State(s)" value={proposal.states.length
            ? proposal.states.map((state) => state.label).join(', ')
            : 'Default state'} />
